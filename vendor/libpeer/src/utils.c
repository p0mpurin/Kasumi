...
void utils_random_string(char* s, const int len) {
  ...
  if (len <= (int)sizeof(random_bytes) && R_SUCCEEDED(psInit())) {
    Result rc = PS_GenerateRandomBytes(random_bytes, (size_t)len);
    psExit();
    if (R_SUCCEEDED(rc)) {
      for (i = 0; i < len; ++i) s[i] = alphanum[random_bytes[i] % (sizeof(alphanum) - 1)];
      s[len] = '\0';
      return;
    }
  }
