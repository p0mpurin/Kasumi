...
static int agent_create_stun_addr(Agent* agent, Address* serv_addr) {
  ...
  agent->gather_responses_received++;
...
static void agent_create_binding_request(Agent* agent, StunMessage* msg) {
  ...
  stun_msg_write_attr(msg, STUN_ATTR_TYPE_USERNAME, strlen(username), username);
  priority_network = htonl((uint32_t)agent->nominated_pair->priority);
...
static int agent_recv_with_timeout(Agent* agent, uint8_t* buf, int len, int timeout_ms) {
  ...
  if ((ret = agent_socket_recv(agent, &addr, buf, len, timeout_ms)) > 0) {
    agent->binding_request_time = ports_get_epoch_time();
    agent->udp_datagrams_received++;
  }
  if (ret > 0 && stun_probe(buf, len) == 0) {
    agent->stun_datagrams_received++;
...
int agent_connectivity_check(Agent* agent) {
  ...
  if (agent->nominated_pair->conncheck % AGENT_CONNCHECK_PERIOD == 0) {
    ...
    agent->connectivity_requests_attempted++;
    if (agent_socket_send(
      ...
      agent->connectivity_requests_sent++;
    } else {
      agent->connectivity_send_failures++;
      agent->connectivity_last_errno = errno;
    }
  }
...
void agent_get_io_stats(Agent* agent, uint32_t* gather_sent, uint32_t* gather_received,
                        ...
                        int* last_send_errno) {
  if (!agent) return;
  if (gather_sent) *gather_sent = agent->gather_requests_sent;
  if (gather_received) *gather_received = agent->gather_responses_received;
  if (checks_sent) *checks_sent = agent->connectivity_requests_sent;
  if (udp_received) *udp_received = agent->udp_datagrams_received;
  if (stun_received) *stun_received = agent->stun_datagrams_received;
  if (valid_responses) *valid_responses = agent->valid_binding_responses;
  if (checks_attempted) *checks_attempted = agent->connectivity_requests_attempted;
  if (send_failures) *send_failures = agent->connectivity_send_failures;
  if (last_send_errno) *last_send_errno = agent->connectivity_last_errno;
}
